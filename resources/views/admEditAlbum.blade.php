...
@section('content')
		...
		<script src="{!! asset('layout/jSortable/jquery-ui.js') !!}"></script>
				...
				$( "#sortable" ).sortable({  
					stop: function( event, ui ) {
						...
						$('#sortable img').each(function() {
							//alert($(this).attr('src'))
							aux= aux + $(this).attr('src') + ";";
						});
					//alert("{{route('admSort',['id'=>'1','vetor_nomes'=>''])}}" + "/" + aux);
					//alert(aux);
					//alert("\"" +{{route('admSort',['id'=>'1'])}} + "\"");
					console.log(aux);
					var fotos = { "nomes_fotos": [aux]};
					$.ajax({
					  type: "POST",
					  url: "{{route('admSort',['id'=>$infos_album->id])}}",
					  data: fotos,//"{nomes: [" + aux + "]}",
        ...
        <div id="adm_control">
                ...
                <span class="file-wrapper">
                   <!--   {!! Form::open(array('url'=>'adm/upload/'.$infos_album->id,'method'=>'POST', 'files'=>true, 'role'=>'form', 'id'=>'Frm_add_fotos_editAlbum')) !!}
                    ...
                    <a href="{{asset('adm/enviafotos/')}}{{'/'.$infos_album->id}}" class="btn btn-success" id="iframe" >Adicionar fotos</a>
                </span>
